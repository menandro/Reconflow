...
void CudaFlow::FlowToHSV(float* u, float * v, int w, int h, int s, float3 * uRGB, float flowscale)
{
...
void CudaFlow::SceneFlowToHSV(float3* sceneflow, int w, int h, int s, float3 * uRGB, float flowscale)
{
	dim3 threads(BlockWidth, BlockHeight);
	dim3 blocks(iDivUp(w, threads.x), iDivUp(h, threads.y));
